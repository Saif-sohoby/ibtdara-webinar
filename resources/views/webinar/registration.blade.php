...
<head>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // Set the "sources" value from query parameter
            const sourceValue = getQueryParam("sources") ?? 'Default';
            if (sourceValue) {
                sourcesInput.value = sourceValue;
            }
                ...
                .catch(error => {
                    if (error.errors) {
                        for (let field in error.errors) {
                            ...
                            errorMessage.innerText = error.errors[field][0];
        ...
        function validateMobile() {
            ...
            const mobileRegex = /^[0-9]{7,15}$/; // Accepts 7 to 15 digits

            if (!mobileRegex.test(mobile)) {
                alert("Please enter a valid mobile number without the country code.");
                return false;
            }

            // Append country code to the mobile number before submission
            mobileInput.value = countryCode + mobile;
            return true;
        }
    </script>
        ...
        <div class="col-md-6">
            <div class="card mt-5">
                <div class="card-header text-center">
                    <h4>{{ $webinar->topic }}</h4>
                    <p><strong>Start:</strong> {{ $webinar->start_time->format('l, d M Y, h:i A') }} 🇸🇦</p>
                    <p><strong>Duration:</strong> {{ $webinar->duration }} minutes</p>
                    <p>Please register below to join the live session.</p>
                </div>
                <div class="card-body">
                    ...
                    <form id="webinar-form" method="POST" action="{{ route('webinar.register.post', ['token' => request()->route('token')]) }}">
                        @csrf
                        <div class="mb-3">
